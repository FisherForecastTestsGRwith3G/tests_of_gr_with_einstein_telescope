...
# get global index
global_index_fisher_full = Dict()
global_index_snr_full = Dict()
global_index_total_full = Dict()
for nn in configs["network_list"]
    file_name = data_folder_name * nn *"/global_indices.h5"
    h5open(file_name, "r") do gi_file
        global_index_fisher_full[nn] = read(gi_file, "fisher")  
        global_index_snr_full[nn] = read(gi_file, "snr")
        global_index_total_full[nn] = read(gi_file, "total")
    end
end

# Load number of catalog and noise realizations requested
    ...
    for realization_index in 1:max_number_of_realizations
        ...
        for idx_noise in 0:configs["n_noise_realizations"]     
            ...
            for pno in configs["pn_waveforms"]
                ...
                # I always assume that the numberOfEventsSingleRealization refers to the events to be drawn from the catalog, and never to the observed events
                # Therefore I comment out the following lines, which in the current implementation would not work
                # (you would need to not select already at most numberOfEventsSingleRealization from the catalog via the global_index_total mask at the start)
                # if configs["numberOfEventsSingleRealization_refer_directly_to_observed_events"]
                ...
                # end
                # Instead I just use the global_index_total to select the events that are used in the current realization, and set n_events_used accordingly
                n_events_used = sum(global_index_total[nn])
            ...
            fig_file_name = output_folder_name * nn * "/all_orders/hyperparamdist_90CI_noise_$(noise_seed)_realization_$(realization_index).pdf"
