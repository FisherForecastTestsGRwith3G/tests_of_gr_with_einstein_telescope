function read_config_grid(config_file::AbstractString)
    config_file = abspath(config_file)
    isfile(config_file) || throw(ArgumentError("Config file does not exist: $(config_file)"))

    raw_config = TOML.parsefile(config_file)

    required_sections = ("general", "catalog", "deviations", "detectors", "fisher", "population", "grid")
    ...
    config["mu_vec"] = Float64.(raw_config["deviations"]["mu_vec"])
    config["sigma_vec"] = Float64.(raw_config["deviations"]["sigma_vec"])
