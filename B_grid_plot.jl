...
needToRun = configs["needToRunAveraging"]
...
if needToRun == true
    ...
    for i in 1:length(mu_vec)
        for j in 1:length(sigma_vec)
            ...
            global res[i,j] = reshuffling_bisection(n_median, wrapper_3sigma, dphi0_k, delta_k, sigma)[1]
        end
    end
...
Colorbar(fig[1, 2], hm,  size = 20,
                     ticklabelsize = 20, ticks = (lim, lim_label))

Label(fig[1, 2, Top()], L"n_{\text{events}}", fontsize = labelfontsize)
